...
function SubmittedProductionComparisonChart() {
  const { data: SubProdCompa } = useGetReportChartDataQuery({
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Submitted Production Comparison",
      },
    },
  };
  ...
  useEffect(() => {
    const newDataSet = [];
    for (const key in SubProdCompa?.data) {
...
export default SubmittedProductionComparisonChart;
